/**
 * 목표 프롬프트를 채팅 입력창의 고스트 텍스트로 렌더링한다 -- 위장의 핵심.
...
export interface ChatGhostInputProps {
  typing: TypingView;
  target: string;
}
...
export function ChatGhostInput({ typing, target }: ChatGhostInputProps) {
  ...
  return (
    <div className="chat-ghost" aria-hidden="true">
      {chars.map((ch, i) => {
        ...
        return (
          <span key={i} className={'chat-ch chat-ch-' + state}>
            {caretInTarget && typing.caret === i ? <span key={'caret-' + typing.value} className="chat-caret" /> : null}
      ...
      {overflow.split('').map((ch, i) => (
        <span
          key={'overflow-' + i}
          className={'chat-ch ' + (ch === ' ' ? 'chat-ch-overflow-space' : 'chat-ch-overflow')}
        >
          {ch}
        </span>
      ))}
      {caretAtTail ? <span key={'caret-' + typing.value} className="chat-caret" /> : null}
    </div>
  );
}
